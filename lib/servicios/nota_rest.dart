...
class NotaRest {
  ...
  static Stream<List<Nota>> listarStream() async* {
    ...
    var url = Uri.http(
        '158.101.30.194', '/testcli/examples/oraclevm2/Note/listall', {});
  ...
  static Future<void> actualizar(Nota nota) async {
    ...
    // http://158.101.30.194/testcli/examples/oraclevm2/Note/listall
    var url = Uri.http(
        '158.101.30.194', '/testcli/examples/oraclevm2/Note/update', {});

    var mapa = nota.toJson();
    Map<String, String> encabezado = {'token': Configuracion.token ?? ''};
    await new Future.delayed(const Duration(seconds: 3));
    var response = await http.post(url,
        body: convert.jsonEncode(mapa), headers: encabezado);
    if (response.statusCode == 200) {
  ...
  static Future<void> insertar(Nota nota) async {
    ...
    // http://158.101.30.194/testcli/examples/oraclevm2/Note/listall
    var url = Uri.http(
        '158.101.30.194', '/testcli/examples/oraclevm2/Note/insert', {});
  ...
  static Stream<Nota> obtenerStream(int id) async* {
    ...
    var url = Uri.http(
        '158.101.30.194', '/testcli/examples/oraclevm2/Note/get/$id', {});
    Map<String, String> encabezado = {'token': Configuracion.token ?? ''};
    //await new Future.delayed(const Duration(seconds : 3));
  ...
  static Future<int> borrarFuture(int id) async {
    ...
    var url = Uri.http(
        '158.101.30.194', '/testcli/examples/oraclevm2/Note/delete/$id', {});
    Map<String, String> encabezado = {'token': Configuracion.token ?? ''};
    //await new Future.delayed(const Duration(seconds : 3));
    var response = await http.get(url, headers: encabezado);
    if (response.statusCode == 200) {
